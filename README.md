...
4.  Save this file as `oracle_api_key.pem` inside the `oci_keys` folder.
5.  Click **Add**.
6.  Copy the content displayed in the "Configuration File Preview" text box.

#### Step B: Config File
1.  Create a file named `config` (with no file extension) inside the `oci_keys` folder.
2.  Paste the content you copied from the Oracle Console into this file.
3.  **Important:** You must edit the `key_file` path in this file to point to the internal container path. Change the line to look exactly like this:
...
```
> **Note:** Do not add a passphrase if you want the process to be fully automated without prompts (optional).
